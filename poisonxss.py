...
def show_help_syntax():
    ...
    print(f"  python poisonxss.py -u <STARTING_URL> -p <PAYLOAD_FILE> [OPTIONS]\n")
...
async def check_for_waf(session, base_url, proxy):
    ...
    try:
        async with session.get(waf_test_url, proxy=proxy, timeout=10) as response:
            ...
            if response.status in (403, 406, 501): return f"Detected (Status Code: {response.status})"
            if any(keyword in text.lower() for keyword in ["waf", "firewall", "forbidden", "blocked", "cloudflare", "incapsula"]): return "Detected (Found keywords in response)"
    except Exception: return "Unknown (Error during check)"
    return "Not Detected"
...
/_/                                               
        """)
        print(f"{Fore.CYAN}PoisonXSS v2.4 [Final]{Style.RESET_ALL} | The All-in-One XSS Scanner")
        ...
        try:
            ...
            if self.crawl:
                ...
                for a_tag in soup.find_all('a', href=True):
                    link = a_tag['href']
                    abs_link = urljoin(url, link).split('#')[0] # Make absolute and remove fragments
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PoisonXSS v2.4", add_help=False)
    
    parser.add_argument('-h', '--help', '--help-syntax', action='store_true', help='Show the detailed help message and exit')
    
    # Core arguments
    core_group = parser.add_argument_group('Core Arguments')
    core_group.add_argument("-u", "--url", help="The starting URL for the scan/crawl")
    core_group.add_argument("-p", "--payloads", help="A file containing XSS payloads")

    # Crawler Options
    crawl_group = parser.add_argument_group('Crawler Options')
    crawl_group.add_argument("--crawl", action="store_true", help="Enable the web crawler")
    ...
    req_group.add_argument("--evade", action="store_true", help="Enable evasion techniques")
    req_group.add_argument("-H", "--headers", help="Custom headers, comma-separated")

    # Control & Finesse
    control_group = parser.add_argument_group('Control & Finesse Options')
    control_group.add_argument("--proxy", help="Route traffic through a proxy (e.g., http://127.0.0.1:8080)")
    control_group.add_argument("-v", "--verbose", action="store_true", help="Display detailed scan output")
    control_group.add_argument("--delay", type=float, default=0, help="Add a delay between requests (seconds)")
    
    # General
    general_group = parser.add_argument_group('General Options')
    general_group.add_argument("-w", "--workers", type=int, default=50, help="Number of concurrent workers")
    general_group.add_argument("-o", "--output", help="Output file for the report")
    ...
    if not payloads:
        print(f"[{Fore.RED}ERROR{Style.RESET_ALL}] No payloads loaded from {args.payloads}")
        return
